...
class Supervisor extends User {
  ...
  assignEmployeeToSupervisor(db, employeeId, callback) {
    const checkSql =
      "SELECT * FROM manages WHERE supervisor_id = ? AND employee_id = ?";
    const insertSql =
      "INSERT INTO manages (supervisor_id, employee_id) VALUES (?, ?)";
    ...
    db.query(checkSql, [this.user_id, employeeId], (err, results) => {
      ...
      if (results.length > 0) {
        // Already exists
        return callback(
          new Error("Employee already assigned to this supervisor.")
        );
      }
  ...
  removeEmployeeFromSupervisor(db, employeeId, callback) {
    const sql =
      "DELETE FROM manages WHERE supervisor_id = ? AND employee_id = ?";
